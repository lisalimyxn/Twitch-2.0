...
const Toggle = ({ on, onToggle, label, sub }: { on: boolean; onToggle?: () => void; label: string; sub?: string }) => (
  <div className="flex items-center justify-between py-3">
    ...
    <button
      onClick={onToggle}
      className={`w-10 h-5 rounded-full relative cursor-pointer transition-colors ${on ? "bg-twitch-purple" : "bg-secondary"}`}
    >
...
interface SafeguardsFlowProps {
  onBack?: () => void;
}

const SafeguardsFlow = ({ onBack }: SafeguardsFlowProps) => {
  const [step, setStep] = useState<SafeguardStep>("creator-optin");
  const [commerceOn, setCommerceOn] = useState(true);
      ...
      <div className="flex-1 overflow-y-auto">
        {step === "creator-optin" && (
          <div className="max-w-2xl mx-auto p-6 space-y-4">
            <div className="flex items-center gap-3">
              {onBack && <button onClick={goBack} className="text-muted-foreground hover:text-foreground"><ArrowLeft className="w-5 h-5" /></button>}
            ...
            <div className="bg-twitch-panel border border-border rounded-lg p-4">
              <Toggle on={commerceOn} onToggle={() => setCommerceOn(!commerceOn)} label="Enable Commerce" sub="Show commerce features in your stream" />
              {commerceOn && (
            ...
            <button onClick={goNext} className="bg-twitch-purple text-primary-foreground px-4 py-2 rounded text-sm font-semibold">
              Next: Viewer Preferences →
            </button>
          </div>
        )}
        ...
        {step === "viewer-prefs" && (
          <div className="max-w-2xl mx-auto p-6 space-y-4">
            ...
            <div className="bg-twitch-panel border border-border rounded-lg p-4 space-y-0">
              ...
              <Toggle on={showCards} onToggle={() => setShowCards(!showCards)} label="Show product cards during streams" sub="See featured products from creators you watch" />
              <div className="border-t border-border" />
              <Toggle on={showNotifs} onToggle={() => setShowNotifs(!showNotifs)} label="Show purchase notifications from other viewers" sub="See when other viewers buy featured products" />
            </div>
            <div className="bg-twitch-purple/10 border border-twitch-purple/30 rounded-lg p-4 text-sm text-muted-foreground">
        ...
        {step === "comparison" && (
          <div className="p-6 space-y-4">
            <div className="flex items-center gap-3">
              <button onClick={goBack} className="text-muted-foreground hover:text-foreground"><ArrowLeft className="w-5 h-5" /></button>
              <h2 className="text-foreground text-2xl font-bold">Side-by-Side Comparison</h2>
            </div>
        ...
        {step === "scale" && (
          <div className="p-6 space-y-4">
            <div className="flex items-center gap-3">
              <button onClick={goBack} className="text-muted-foreground hover:text-foreground"><ArrowLeft className="w-5 h-5" /></button>
              <h2 className="text-foreground text-2xl font-bold">Product Card at Actual Scale</h2>
            </div>
